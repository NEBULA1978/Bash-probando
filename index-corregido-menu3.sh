...
customize_settings() {
  while true; do
    show_default_options
    echo "Seleccione una opción para cambiar (1-${#OPTIONS[@]}), o escriba 'terminar' para finalizar:"
    read -r selected_option

    if [[ "$selected_option" == "terminar" ]]; then
      break
    elif [[ "$selected_option" -ge 1 && "$selected_option" -le ${#OPTIONS[@]} ]]; then
      selected_index=$((selected_option-1))
      default_value=${DEFAULT_VALUES[selected_index]}
      echo "Seleccionaste: $((selected_index+1)). ${OPTIONS[selected_index]}"
      echo "Introduce un nuevo valor para la opción (actual: '$default_value'):"
      read -r user_input
      DEFAULT_VALUES[selected_index]=$user_input
    else
      echo "Opción no válida. Por favor, seleccione un número entre 1 y ${#OPTIONS[@]} o escriba 'terminar' para finalizar."
    fi
  done
}
